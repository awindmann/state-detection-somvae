...
def plot_state_prediction(model, start_idx=None, stop_idx=None, step=1, title=None, width=500, height=400):
    ...
    pio.write_image(fig, f"figures/state-plot.pdf", width=width, height=height)
    fig.show()
...
def run_visualization():
    logdir = "../logs/SOMVAE"
    model_name = "final"
    model = load_model(logdir, model_name)
    plot_state_prediction(model, title="Fill levels with predicted state")
