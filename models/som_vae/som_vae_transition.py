...
from models.vae_ts_test.models import LinearDecoder, LinearEncoder, RNNDecoder, RNNEncoder


class SOMVAE(pl.LightningModule):
    def __init__(self, d_input=100, d_channel=3, d_enc_dec=100, with_transition=False, with_rnn=False,
                 d_latent=64, d_som=[3, 3],
        ...
        # INFO: for the transition model to learn, the samples have to be loaded in order
        self.with_transition = with_transition
        if not with_transition:
            self.hparams.gamma = 0
            self.hparams.tau = 0
        ...
        if not with_rnn:
            ...
            self.encoder = RNNEncoder(input_size=d_channel, hidden_size=d_latent, rnn_layers=5)
            self.decoder_e = RNNDecoder(input_size=d_channel, seq_len=d_input, latent_dim=d_latent,
                                        rnn_layers=5, hidden_size=10)
            self.decoder_q = RNNDecoder(input_size=d_channel, seq_len=d_input, latent_dim=d_latent,
                                        rnn_layers=5, hidden_size=10)
    ...
    def _shared_step(self, x):
        ...
        loss, raw_loss, prob_loss = self.loss(x, x_e, x_q, z_e, z_q, z_q_neighbors, k, z_dist)
        return loss, raw_loss, prob_loss
    ...
    def loss(self, x, x_e, x_q, z_e, z_q, z_q_neighbors, k, z_dist_flat):
        ...
        loss = mse_l + self.hparams.alpha * commit_l + self.hparams.beta * som_l \
            + self.hparams.gamma * prob_l + self.hparams.tau * prob_z_l  # 2x gamma similar to original repository
        raw_loss = mse_l + commit_l + som_l \
                   + prob_l + prob_z_l
        return loss, raw_loss, prob_l
    ...
    def training_step(self, batch, batch_id):
        loss, raw_loss, prob_loss = self._shared_step(batch)
    ...
    def on_train_epoch_end(self, unused=None) -> None:
        """normalizes transition matrix
        Thus, transition matrix still contains transition probabilities.
        """
        if self.with_transition:
            probs_pos = torch.exp(self.probs)
            probs_sum = torch.sum(probs_pos, dim=[-1, -2], keepdim=True)
    ...
    def validation_step(self, batch, batch_id):
        ...
        self.logger.log_hyperparams(self.hparams, {"hp/val_loss_raw": raw_loss})  # log hparams with val_loss as reference
